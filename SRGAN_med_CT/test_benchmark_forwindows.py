...
def main():
    ...
    # 默认修改为epoch_2_78.pth(G为生成器,D为判别器(只在训练时使用))
    #parser.add_argument('--model_name', default='netG_epoch_2_78.pth', type=str, help='generator model epoch name')
    # 默认修改为epoch_4_40.pth(G为生成器,D为判别器(只在训练时使用))
    # parser.add_argument('--model_name', default='netG_epoch_4_40.pth', type=str, help='generator model epoch name')
    # 尝试修改为epoch_8_89.pth(G为生成器,D为判别器(只在训练时使用))
    parser.add_argument('--model_name', default='netG_epoch_8_89.pth', type=str, help='generator model epoch name')
