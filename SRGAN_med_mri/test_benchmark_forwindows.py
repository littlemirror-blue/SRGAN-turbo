...
def main():
    ...
    # 默认修改为epoch_2_59.pth(G为生成器,D为判别器(只在训练时使用))
    #parser.add_argument('--model_name', default='netG_epoch_2_59.pth', type=str, help='generator model epoch name')
    # 默认修改为epoch_4_80.pth(G为生成器,D为判别器(只在训练时使用))
    #parser.add_argument('--model_name', default='netG_epoch_4_80.pth', type=str, help='generator model epoch name')
    # 尝试修改为epoch_8_83.pth(G为生成器,D为判别器(只在训练时使用))
    parser.add_argument('--model_name', default='netG_epoch_8_83.pth', type=str, help='generator model epoch name')
